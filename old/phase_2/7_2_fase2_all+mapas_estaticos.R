...
# Número de clusters
n_clusters <- length(unique(na.omit(locality_coords$Cluster)))
n_clusters
# Paleta extendida
pal <- colorRampPalette(brewer.pal(8, "Set2"))(n_clusters)

# Convertir puntos y edges a objetos sf para manejarlos con ggplot + mapas
points_sf <- st_as_sf(locality_coords, coords = c("lon", "lat"), crs = 4326)

edges_list <- lapply(1:nrow(edges_mapa), function(i) {
  st_linestring(matrix(
    c(edges_mapa$lon_from[i], edges_mapa$lat_from[i],
      edges_mapa$lon_to[i],   edges_mapa$lat_to[i]),
    ncol = 2, byrow = TRUE
  ))
})
edges_sf <- st_sf(
  geometry = st_sfc(edges_list, crs = 4326),
  peso = edges_mapa$peso
)

# --- Mapa con fondo y colores de clusters ---
p <- ggplot() +
  # Fondo (puedes cambiar provider = ... a "CartoDB.Positron", "OpenStreetMap", etc.)
  annotation_map_tile(type = "cartolight", zoomin = -1) + 
  
  # Conexiones
  #geom_sf(data = edges_sf, aes(linewidth = peso), color = "gray40", alpha = 0.5) +
  
  # Puntos (relleno por cluster, borde negro)
  geom_sf(data = points_sf, aes(fill = factor(Cluster)), color = "black",
          shape = 21, size = 4, stroke = 0.7) +
  ...
  labs(title = "Network of Localities (Protists)")

p


# --- Guardar ---
ggsave("mapa_localidades_conexiones_protist.png",
       plot = p, width = 16, height = 12, dpi = 1200)

ggsave("mapa_localidades_metazoos_unconnected.pdf",
       plot = p,
       width = 16,
       height = 12)
...
best_k <- 10 #to force k=10 to make the clusters comparable bwt kmeans and clustalgeo
...
#__________________________________________Etiquetas clusters (añadir con AI)
# -----------------------------
# Preparar datos para ggplot
# -----------------------------
# Filtrar y escalar grosor de las líneas
edges_plot <- edges_mapa_kmeans %>%
  filter(!is.na(lon_from) & !is.na(lon_to)) %>%
  mutate(weight_scaled = rescale(peso, to = c(0.5, 2)))  # ajustar grosor

# Definir paleta de colores según número de clusters
n_clusters_km <- length(unique(na.omit(locality_coords_kmeans$Cluster)))
if (n_clusters_km <= 9) {
  pal_km <- brewer.pal(n_clusters_km, "Set2")
} else {
  pal_km <- colorRampPalette(brewer.pal(8, "Set2"))(n_clusters_km)
}

# Calcular centroides basados en los resultados del k-means
centroids <- locality_coords_kmeans %>%
...
# Crear mapa estático
# -----------------------------
mapa_kmeans_tags <- ggplot() +
  
  # Puntos de localidades
  geom_point(
    data = locality_coords_kmeans,
    aes(x = lon, y = lat, color = factor(Cluster)),
    size = 3
  ) +
  
  # Etiquetas de clusters en centroides del k-means
  geom_label_repel(
    data = centroids,
    aes(x = lon, y = lat, label = Cluster, fill = factor(Cluster)),
    color = "white", fontface = "bold", size = 4,
    box.padding = 0.3, point.padding = 0.5,
    segment.color = NA
  ) +
  
  # Escala de color por cluster
  scale_color_manual(values = pal_km, name = "Clusters K-means") +
  scale_fill_manual(values = pal_km, guide = "none") +   # mismo fill que color, pero sin segunda leyenda
  
  # Tema limpio y etiquetas
  theme_minimal() +
  labs(
    title = "Mapa de clusters K-means",
    x = "Longitud",
    y = "Latitud"
  ) +
...
# -----------------------------
# Guardar mapa como PNG (alta resolución)
# -----------------------------
ggsave(
  filename = "kmeans_fungi_k7_clusterstag.png",
  plot = mapa_kmeans_static,
  width = 16,
  height = 12,
  dpi = 1200
)
...
# --- Guardar mapa en PDF ---
# Nota: los tiles se incrustan como raster en el PDF
ggsave("kmeans_metazoos_k10_clusterstag.pdf",
       mapa_kmeans_tags,
       width = 16, height = 12)

#________________________________________________________MAPA SIN LAS ETIQUETAS DE CLUSTERS
# -----------------------------
# Preparar datos como objetos sf
# -----------------------------
# Puntos
points_sf <- st_as_sf(locality_coords_kmeans, coords = c("lon", "lat"), crs = 4326)

# Líneas
edges_list <- lapply(1:nrow(edges_mapa_kmeans), function(i) {
  st_linestring(matrix(
    c(edges_mapa_kmeans$lon_from[i], edges_mapa_kmeans$lat_from[i],
      edges_mapa_kmeans$lon_to[i],   edges_mapa_kmeans$lat_to[i]),
    ncol = 2, byrow = TRUE
  ))
})
edges_sf <- st_sf(
  geometry = st_sfc(edges_list, crs = 4326),
  peso = edges_mapa_kmeans$peso
)

# Escalar grosor de líneas
edges_sf$weight_scaled <- scales::rescale(edges_sf$peso, to = c(0.5, 2))
...
# -----------------------------
# Crear mapa con fondo 
# -----------------------------
mapa_kmeans_sf <- ggplot() +
  # Fondo de mapa
  annotation_map_tile(type = "cartolight", zoomin = -1) +
  
  # Líneas de conexión
  #geom_sf(data = edges_sf, aes(linewidth = weight_scaled), 
  ...
  # Escalas
  scale_fill_manual(values = pal_km, name = "Clusters K-means") +
  ...
  labs(
    title = "K-means Clusters Arqueaplastidia",
    x = "Longitud",
    y = "Latitud"
  ) +
  # Forzar que muestre ejes en grados
  coord_sf()
...
# Guardar mapa
# -----------------------------
ggsave("mapa_kmeans_arqueaplastidia_k10_connections.png",
       plot = mapa_kmeans_sf, width = 16, height = 12, dpi = 1200)

ggsave("kmeans_metazoos_k10_unconnected.pdf",
       plot = mapa_kmeans_sf, width = 16, height = 12)
...
#----------------------------------
#________________________________________________________________________MAPA SIN CONEXIONES
library(ggplot2)
library(dplyr)
library(scales)
library(RColorBrewer)
...
# Pasamos a objeto sf
points_sf <- st_as_sf(localidades_clust, coords = c("lon", "lat"), crs = 4326)

# Número de clusters y paleta
n_clusters_cg <- length(unique(points_sf$Cluster))
pal_cg <- if (n_clusters_cg <= 8) {
  brewer.pal(n_clusters_cg, "Set2")
} else {
  scales::hue_pal()(n_clusters_cg)
}

# -----------------------------
# Crear mapa con fondo y etiquetas
# -----------------------------
mapa_clustgeo_sf <- ggplot() +
  # Fondo de mapa
  annotation_map_tile(type = "cartolight", zoomin = -1) +
  ...
  # Escalas
  scale_fill_manual(values = pal_cg, name = "Clusters ClustGeo") +
  scale_color_manual(values = pal_cg) +
  
  theme_minimal(base_size = 14) +
  theme(panel.grid = element_blank(),
        legend.position = "right") +
  labs(title = paste("Mapa de clusters ClustGeo (k =", best_k_clustgeo, ")"))
...
# Guardar mapa
# -----------------------------
ggsave("mapa_clustgeo_fungi_k3_unconnected.png",
       plot = mapa_clustgeo_sf, width = 16, height = 12, dpi = 1200)

ggsave("clustgeo_metazoos_k10_unconnected.pdf",
       plot = mapa_clustgeo_sf, width = 16, height = 12)


#________________________________________________________________________MAPA CON CONEXIONES
# -----------------------------
# Preparar datos con clusters ClustGeo
# -----------------------------
clust_opt <- cutree(tree_opt, k = best_k_clustgeo)
...
localidades_clust <- localidades %>%
  mutate(Cluster = factor(clust_opt))   # convierto a factor aquí

points_sf <- st_as_sf(localidades_clust, coords = c("lon", "lat"), crs = 4326)

# -----------------------------
# Preparar líneas de conexión
# -----------------------------
edges_list <- lapply(1:nrow(edges_locality_net), function(i) {
  st_linestring(matrix(
    c(
      localidades_clust$lon[localidades_clust$Localities == edges_locality_net$Locality_from[i]],
      localidades_clust$lat[localidades_clust$Localities == edges_locality_net$Locality_from[i]],
      localidades_clust$lon[localidades_clust$Localities == edges_locality_net$Locality_to[i]],
      localidades_clust$lat[localidades_clust$Localities == edges_locality_net$Locality_to[i]]
    ),
    ncol = 2, byrow = TRUE
  ))
})
...
edges_sf <- st_sf(
  geometry = st_sfc(edges_list, crs = 4326),
  peso = edges_locality_net$dist_gen_prom
)
...
mapa_clustgeo_sf <- ggplot() +
  ...
  # Tema y ejes con coordenadas
  theme_minimal(base_size = 14) +
  theme(panel.grid = element_line(color = "gray80", size = 0.2),
        legend.position = "right") +
  labs(
    title = paste("Mapa de clusters ClustGeo (k =", best_k_clustgeo, ")"),
    x = "Longitud",
    y = "Latitud"
  )
...
# Exportar
# -----------------------------
ggsave("clustgeo_metazoos_k10_connections.pdf",
       plot = mapa_clustgeo_sf, width = 16, height = 12)



#_____________________________________________ETIQUETAS CLUSTERS (añadir en AI)
# -----------------------------
# Localidades y clusters
# -----------------------------
# Cortar el árbol con el k óptimo (o el que quieras visualizar)
clust_opt <- cutree(tree_opt, k = best_k_clustgeo)

localidades_clust <- localidades %>%
  mutate(Cluster = factor(clust_opt))

# Pasamos a objeto sf
points_sf <- st_as_sf(localidades_clust, coords = c("lon", "lat"), crs = 4326)

# -----------------------------
# Centroides de clusters para etiquetar
# -----------------------------
centroids_sf <- points_sf %>%
  group_by(Cluster) %>%
...
# -----------------------------
# Definir el bounding box (margen automático)
# -----------------------------
points_buffer <- st_buffer(points_sf, dist = 0.5)
bb_buffer <- st_bbox(points_buffer)

# -----------------------------
# Mapa estático con ggplot
# -----------------------------
mapa_clustgeo_tags <- ggplot() +
  # Fondo
  #annotation_map_tile(type = "cartolight", zoomin = -1) +
  
  # Puntos
  geom_sf(data = points_sf, aes(fill = Cluster), color = "black",
          shape = 21, size = 4, stroke = 0.7) +
  
  # Etiquetas
  geom_label_repel(data = centroids,
                   aes(x = lon, y = lat, label = Cluster, fill = Cluster),
                   color = "white", fontface = "bold", size = 4,
                   box.padding = 0.3, point.padding = 0.5,
                   segment.color = NA) +
  ...
  # Centrado automático
  coord_sf(xlim = c(bb_buffer["xmin"], bb_buffer["xmax"]),
           ylim = c(bb_buffer["ymin"], bb_buffer["ymax"])) +
...
# Exportar
# -----------------------------
ggsave("clustgeo_metazoos_clustertags.pdf",
       plot = mapa_clustgeo_tags, width = 16, height = 12)
